define(['./module'],function(controllers){
    'use strict';
    controllers.controller('UpdateRouteCtrl', ['$scope','$timeout','$http','fb','imageService','Upload', function($scope, $timeout, $http, fb, imageService, Upload) {
    ...
    $http({
        ...
        $scope.data = data;
        $('#complexity-select option').filter(function() {
            return ($(this).val() == $scope.data.complexity);
        }).prop('selected', true);
        $scope.data.chosenPlaces = $scope.data.places;
        initMap(48.3959259, 23.9243141);
        calculateAndDisplayRoute($scope.data.places)
    })
